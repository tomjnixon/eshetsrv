...
start_stop_test_() ->
    {"the server can be started and stopped", ?setup(fun nothing/1)}.
...
register_test_() ->
    [
        {"register action", ?setup(fun register_action/1)},
        {"register prop", ?setup(fun register_prop/1)},
        {"register event then listener", ?setup({with, [fun register_event_first/1]})},
        {"register listener then event", ?setup(fun register_listener_first/1)}
    ].
...
state_test_() ->
    [
        {"register state", ?setup({with, [fun register_state/1]})},
        {"observe before register state", ?setup({with, [fun observe_before_register_state/1]})}
    ].
...
link_test_() ->
    [
        {"register then exit immediately", ?setup({with, [fun register_exit/1]})},
        {"register, check, then exit", ?setup({with, [fun register_check_exit/1]})},
        {"state", ?setup({with, [fun link_state/1]})}
    ].
...
register_exit(Pid) ->
    ...
    receive
        {_Tag, MonitorRef, _Type, _Object, _Info} ->
            timer:sleep(50),
            ?_assertEqual(
                {error, path_not_found}, eshetsrv_state:lookup(Pid, action_owner, <<"/foo">>)
            )
    end.
    ...
    ok =
        receive
            {_Tag, MonitorRef, _Type, _Object, _Info} ->
                ...
                after 100 -> state_changed_timeout
                end
        after 100 -> monitor_timeout
        end.
